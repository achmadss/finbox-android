...
data class ExtensionDetailScreen(private val id: String) : Screen {

    @Composable
    override fun Content() {
        val navigator = LocalNavigator.currentOrThrow
        val model = rememberScreenModel(tag = id) { ExtensionDetailScreenModel(id) }
    ...
    Scaffold(
        ...
        if (extension == null) {
            Box(Modifier.fillMaxSize().padding(padding), Alignment.Center) {
                Text(
                    text = stringResource(R.string.extension_unknown),
        ...
        Column(
            ...
            Column(
                ...
                ExtensionIcon(extension.id, Modifier.size(112.dp))
            ...
            // No version and no size: an extension has no release of its own to
...
/**
 * What this extension can read, asked of the class rather than of anything it
 ...
 */
@Composable
private fun SourceRow(extension: Extension) {
    val sources = buildList {
        if (extension.source is EmailSource) add(stringResource(R.string.source_email))
    }
    Column(Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 12.dp)) {
        Text(
            text = stringResource(R.string.extension_reads),
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        Text(
...
@Preview
@Composable
private fun ExtensionDetailUnknownPreview() {
    AppTheme {
        ExtensionDetailScreenContent(
            state = ExtensionDetailScreenModel.State(extension = null, enabled = false),
        )
    }
}
