...
object SourcesScreen : Screen {
    ...
    private fun readResolve(): Any = SourcesScreen
    ...
    override fun Content() {
        ...
        val model = rememberScreenModel { SourcesScreenModel() }
        val state by model.state.collectAsState()

        SourcesScreenContent(
            state = state,
            onBack = navigator::pop,
            onOpenSource = { navigator.push(SourceDetailScreen(it)) },
...
private fun SourceListItem(
    ...
    Row(
        ...
        SourceIcon(row.source, Modifier.size(40.dp))
...
/**
 * The source's own icon, out of the source's own module.
 *
 * No fallback and no lookup by id: `icon` is a property every source has to
 * declare, so a missing one is a compile error in that source's module rather
...
private fun SourcesScreenPreview() {
    AppTheme {
        SourcesScreenContent(state = SourcesScreenModel.State(rows = emptyList()))
    }
}
