...
/**
 * Which of a parser's transaction kinds the user has switched off.
 *
 * Stored as the off ones rather than the on ones so a kind a parser adds in
 ...
 */
class ParserKindPreference(
    private val preferenceStore: PreferenceStore,
) {

    fun disabled(pkg: String): Preference<Set<String>> =
        preferenceStore.getStringSet("parser_disabled_kinds_$pkg")
