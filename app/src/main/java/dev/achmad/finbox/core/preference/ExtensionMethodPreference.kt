 ...
 *
 * Stored as the off ones so a method an extension adds later arrives switched on;
 * keyed by package, not extension id, since the id encodes the version code and an
 * update would reset every switch.
 */
class ExtensionMethodPreference(
    private val preferenceStore: PreferenceStore,
) {

    fun disabled(pkg: String): Preference<Set<String>> =
        preferenceStore.getStringSet("extension_disabled_methods_$pkg")
