...
class AccountRepositoryImpl(
    ...
    override fun accounts(): Flow<List<EmailAccount>> =
        db.accountQueries.SELECTAllAccounts()
            .asFlow()
            .mapToList(Dispatchers.IO)
            .map { rows -> rows.map { it.toModel() } }

    override suspend fun getById(id: String): EmailAccount? = withContext(Dispatchers.IO) {
        db.accountQueries.SELECTById(id).executeAsOneOrNull()?.toModel()
    }

    override suspend fun upsert(account: EmailAccount) = withContext(Dispatchers.IO) {
